...
ActiveRecord::Schema[8.0].define(version: 2026_04_04_000006) do
  ...
  create_table 'account_conversations', force: :cascade do |t|
    ...
    t.index %w(account_id conversation_id participant_account_ids), name: 'index_unique_conversations', unique: true
  ...
  create_table 'account_statuses_cleanup_policies', force: :cascade do |t|
    ...
    t.integer 'min_status_age', default: 1_209_600, null: false
  ...
  create_table 'accounts', id: :bigint, default: -> { "timestamp_id('accounts'::text)" }, force: :cascade do |t|
    ...
    t.index "(((setweight(to_tsvector('simple'::regconfig, (display_name)::text), 'A'::\"char\") || setweight(to_tsvector('simple'::regconfig, (username)::text), 'B'::\"char\")) || setweight(to_tsvector('simple'::regconfig, (COALESCE(domain, ''::character varying))::text), 'C'::\"char\")))",
            name: 'search_index', using: :gin
  ...
  create_table 'announcement_reactions', force: :cascade do |t|
    ...
    t.datetime 'created_at', precision: nil, null: false
    t.datetime 'updated_at', precision: nil, null: false
    t.index %w(account_id announcement_id name), name: 'index_announcement_reactions_on_account_id_and_announcement_id', unique: true
  ...
  create_table 'custom_feed_configs', force: :cascade do |t|
    t.bigint 'account_id', null: false
    t.bigint 'list_id', null: false
    t.boolean 'enabled', default: true, null: false
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.integer 'pull_cadence_minutes', default: 15, null: false
    t.datetime 'last_pulled_at'
    t.index ['account_id'], name: 'index_custom_feed_configs_on_account_id'
    t.index ['last_pulled_at'], name: 'index_custom_feed_configs_on_last_pulled_at'
    t.index ['list_id'], name: 'index_custom_feed_configs_on_list_id', unique: true
  end

  create_table 'custom_feed_pull_cursors', force: :cascade do |t|
    t.bigint 'custom_feed_step_id', null: false
    t.string 'bucket', default: '', null: false
    t.string 'last_fetched_id'
    t.datetime 'last_fetched_at'
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index ['custom_feed_step_id', 'bucket'], name: 'index_pull_cursors_step_bucket', unique: true
  end

  create_table 'custom_feed_steps', force: :cascade do |t|
    t.bigint 'custom_feed_config_id', null: false
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w(custom_feed_config_id phase position), name: 'index_custom_feed_steps_on_config_phase_position'
    t.index %w(custom_feed_config_id phase step_type), name: 'index_custom_feed_steps_unique_type_per_phase', unique: true
    t.index ['custom_feed_config_id'], name: 'index_custom_feed_steps_on_custom_feed_config_id'
  end
  ...
  create_table 'notifications', force: :cascade do |t|
    ...
    t.index %w(account_id id type), name: 'index_notifications_on_account_id_and_id_and_type', order: { id: :desc }
    t.index %w(account_id id type), name: 'index_notifications_on_filtered', order: { id: :desc }, where: '(filtered = false)'
  ...
  create_table 'quotes', id: :bigint, default: -> { "timestamp_id('quotes'::text)" }, force: :cascade do |t|
    ...
    t.index %w(account_id quoted_account_id id), name: 'index_quotes_on_account_id_and_quoted_account_id_and_id'
  ...
  create_table 'severed_relationships', force: :cascade do |t|
    ...
    t.index %w(relationship_severance_event_id local_account_id direction remote_account_id), name: 'index_severed_relationships_on_unique_tuples', unique: true
  ...
  create_table 'statuses', id: :bigint, default: -> { "timestamp_id('statuses'::text)" }, force: :cascade do |t|
    ...
    t.index %w(account_id id visibility updated_at), name: 'index_statuses_20190820', order: { id: :desc }, where: '(deleted_at IS NULL)'
    t.index ['account_id'], name: 'index_statuses_on_account_id'
    t.index ['conversation_id'], name: 'index_statuses_on_conversation_id'
    t.index ['deleted_at'], name: 'index_statuses_on_deleted_at', where: '(deleted_at IS NOT NULL)'
    t.index ['id', 'account_id'], name: 'index_statuses_local_20190824', order: { id: :desc }, where: '((local OR (uri IS NULL)) AND (deleted_at IS NULL) AND (visibility = 0) AND (reblog_of_id IS NULL) AND ((NOT reply) OR (in_reply_to_account_id = account_id)))'
    t.index %w(id language account_id), name: 'index_statuses_public_20250129', order: { id: :desc }, where: '((deleted_at IS NULL) AND (visibility = 0) AND (reblog_of_id IS NULL) AND ((NOT reply) OR (in_reply_to_account_id = account_id)))'
  ...
  add_foreign_key 'custom_feed_configs', 'accounts'
  add_foreign_key 'custom_feed_configs', 'lists'
  add_foreign_key 'custom_feed_pull_cursors', 'custom_feed_steps'
  add_foreign_key 'custom_feed_steps', 'custom_feed_configs'
  ...
  create_view 'account_summaries', materialized: true, sql_definition: <<~SQL.squish
  ...
  SQL
  add_index 'account_summaries', %w(account_id language sensitive), name: 'idx_on_account_id_language_sensitive_250461e1eb'
  add_index 'account_summaries', ['account_id'], name: 'index_account_summaries_on_account_id', unique: true

  create_view 'global_follow_recommendations', materialized: true, sql_definition: <<~SQL.squish
  ...
  create_view 'instances', materialized: true, sql_definition: <<~SQL.squish
  ...
  create_view 'user_ips', sql_definition: <<~SQL.squish
      SELECT t0.user_id,
      t0.ip,
